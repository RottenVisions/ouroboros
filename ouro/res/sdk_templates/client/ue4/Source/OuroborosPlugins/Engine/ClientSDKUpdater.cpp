...
void ClientSDKUpdater::onImportClientSDK(const UOBEventData* eventData)
{
	if (eventData == nullptr) return;

	const UOBEventData_onImportClientSDK* pEventData = dynamic_cast<const UOBEventData_onImportClientSDK*>(eventData);
	...
	if (pSdkFileStream->length() == pEventData->fileSize)
	{
		...
		if (pEventData->remainingFiles == 0)
		{
			...
			replaceNewSDK();
			UE_LOG(LogTemp, Warning, TEXT("End Update OuroborosPlugin!"));

			UOBEventData_onImportClientSDKSuccessfully* pEventData = NewObject<UOBEventData_onImportClientSDKSuccessfully>();

			OUROBOROS_EVENT_FIRE(OBEventTypes::onImportClientSDKSuccessfully, pEventData);
		}
	}
}
...
void ClientSDKUpdater::downloadSDKFromServer()
{
	...
	if (deleteDirectory(sdkTempPath))
	{
		...
		int clientWindowSize = (int)OuroborosApp::getSingleton().getInitArgs()->TCP_RECV_BUFFER_MAX;
		...
		bundle->send(OuroborosApp::getSingleton().pNetworkInterface());
...
{
	warnUpdateSDK = "Version does not match the server.\nClick to update OuroborosPlugin!\nPull from: " + OuroborosApp::getSingleton().getInitArgs()->ip + ":" + FString::FromInt(OuroborosApp::getSingleton().getInitArgs()->port);
...
{
	OUROBOROS_REGISTER_EVENT("onImportClientSDK", onImportClientSDK);
}
...
void ClientSDKUpdater::replaceNewSDK()
{
	...
	FString tempScriptPath = "/Source/OuroborosPlugins/Scripts";
