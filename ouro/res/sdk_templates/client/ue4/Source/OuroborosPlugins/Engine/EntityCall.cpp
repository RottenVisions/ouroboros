...
{
	if(OuroborosApp::getSingleton().currserver() == TEXT("loginapp"))
	{
		ERROR_MSG("%s::newCall(%s), currserver=%s!", *className, *methodName, *OuroborosApp::getSingleton().currserver());  
		return NULL;
	}
...
void EntityCall::sendCall(Bundle* inBundle)
{
	...
	inBundle->send(OuroborosApp::getSingleton().pNetworkInterface());
