...
bool NetworkInterfaceKCP::initKCP()
{
	...
	ikcp_wndsize(pKCP_, OuroborosApp::getSingleton().getInitArgs()->getUDPSendBufferSize(), OuroborosApp::getSingleton().getInitArgs()->getUDPRecvBufferSize());
...
void NetworkInterfaceKCP::tickConnecting()
{
	...
	if(socket_->HasPendingData(DataSize))
	{
		...
		if (socket_->RecvFrom(s.data(), s.size(), BytesRead, *remoteAddr))
		{
			...
			else if (OuroborosApp::getSingleton().serverVersion() != versionString)
			{
				ERROR_MSG("NetworkInterfaceKCP::connectTo(): failed to connect to '%s:%d'! version(%s!=%s) mismatch!",
					*connectIP_, connectPort_, *versionString, *OuroborosApp::getSingleton().serverVersion());
			...
			UOBEventData_onConnectionState* pEventData = NewObject<UOBEventData_onConnectionState>();
			pEventData->success = success;
			pEventData->address = FString::Printf(TEXT("%s:%d"), *connectIP_, connectPort_);
			OUROBOROS_EVENT_FIRE(OBEventTypes::onConnectionState, pEventData);
		}
	}
	else
	{
