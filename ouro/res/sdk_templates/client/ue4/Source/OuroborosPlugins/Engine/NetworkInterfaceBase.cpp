...
void NetworkInterfaceBase::close()
{
	if (socket_)
	{
		...
		OUROBOROS_EVENT_FIRE(OBEventTypes::onDisconnected, NewObject<UOBEventData_onDisconnected>());
	}

	socket_ = NULL;

	OURO_SAFE_RELEASE(pPacketSender_);
	OURO_SAFE_RELEASE(pPacketReceiver_);
...
void NetworkInterfaceBase::tickConnecting()
{
	...
	{
		// If the connection times out, the callback fails
