namespace Ouroboros
{
	...
	/*
  entitydef the support of basic data types
  The modified module class abstract out of all the support types and provides these types of data are serialized
  into a binary data and the deserialization operation is mainly used for network communication packaged with solution packages
	*/
	...
	public class DATATYPE_ARRAY : DATATYPE_BASE
	{
		...
		{
			if(vtype.GetType ().BaseType.ToString() == "Ouroboros.DATATYPE_BASE")
	...
	public class DATATYPE_FIXED_DICT : DATATYPE_BASE
	{
		...
		public override void bind()
		{
			...
			foreach(string itemkey in keys)
			{
				...
				if(type.GetType ().BaseType.ToString() == "Ouroboros.DATATYPE_BASE")
