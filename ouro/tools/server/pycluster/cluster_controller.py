...
# FLOAT;Unit: second
# Each time the query fails, the number of retries is invalid.
...
def initRootPath():
	"""
	Initialize the root directory to load other scripts
	"""
...
class ClusterStopHandler(ClusterControllerHandler):
	...
	def sendStop(self, showDebug):
		...
		for ctype in self.startTemplate:
			...
			# It is best to try as many times as possible, otherwise the package may not be restored in time to cause subsequent query confusion.
...
class ClusterSaveProcessHandler(ClusterControllerHandler):
	"""
	Store the current server running state into the configuration so that it can be started directly next time using this process
	"""
	# Valid componentType
	VALIDATE_CT = set( [
			DBMGR_TYPE,
...
class ClusterLoadProcessHandler(ClusterControllerHandler):
	"""
	Start the server from the configuration file
	"""
	# Valid componentType
	VALIDATE_CT = set( [
			DBMGR_TYPE,
...
class ClusterStartServerHandler(ClusterControllerHandler):
	"""
	Quickly start a group of servers locally (for testing)
	"""
	# componentType to run
	VALIDATE_CT = [
			LOGGER_TYPE,
			INTERFACES_TYPE,
			DBMGR_TYPE,
...
if __name__ == "__main__":
	...
	if len(sys.argv)  >= 2:
		...
		# python cluster_controller.py sendlog uid NORMAL|INFO|DEBUG|ERROR|WARNING logstr
			...
			#if no UID parameter is given, the default is 0, query all machines
			if uid < 0:
				uid = 0
