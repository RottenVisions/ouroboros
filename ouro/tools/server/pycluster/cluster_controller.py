...
# FLOAT；Unit: seconds
# Each query Machine failed the number of retries
...
def initRootPath():
	"""
	Initialize the root directory to load the other scripts
	"""
...
class ClusterStartHandler(ClusterControllerHandler):
	def __init__(self, uid, startTemplate, machineIP, cid, gus, ouro_root, ouro_res_path, ouro_bin_path):
		ClusterControllerHandler.__init__(self, uid)
		...
		self.cid = cid
		self.gus = gus
		self.ouro_root = ouro_root
		self.ouro_res_path = ouro_res_path
		self.ouro_bin_path = ouro_bin_path
	...
	def do(self):
		...
		for ctype in self.startTemplate:
			...
			self.startServer( COMPONENT_NAME2TYPE[ctype], self.cid, self.gus, self.machineIP, self.ouro_root, self.ouro_res_path, self.ouro_bin_path )
...
class ClusterStopHandler(ClusterControllerHandler):
	...
	def sendStop(self, showDebug):
		...
		for ctype in self.startTemplate:
			...
			# It is best to try many attempts, otherwise it may delay in recovery caused by the subsequent query confusion
...
class ClusterSaveProcessHandler(ClusterControllerHandler):
	"""
	The current server run status is stored to the configuration, so that the next time you can use this process started directly
	"""
	# Effective componentType
	VALIDATE_CT = set( [
			DBMGR_TYPE,
			LOGINAPP_TYPE,
			BASEAPPMGR_TYPE,
...
class ClusterLoadProcessHandler(ClusterControllerHandler):
	"""
	From the configuration file to start the server
	"""
	# Effective componentType
	VALIDATE_CT = set( [
			DBMGR_TYPE,
...
class ClusterStartServerHandler(ClusterControllerHandler):
	"""
	In the local Quick start a group of servers for testing
	"""
	# You want to run componentType
	VALIDATE_CT = [
			LOGGER_TYPE,
			INTERFACES_TYPE,
			DBMGR_TYPE,
		...
		# python cluster_controller.py sendlog uid NORMAL|INFO|DEBUG|ERROR|WARNING logstr
			...
			if len(sys.argv) < 3:
				...
				if sys.argv[2].isdigit():
					...
					print("exp: cluster_controller.py save 501 ouroboros_process.cfg")
					exit(1)
				filename = sys.argv[3]
			...
			if len(sys.argv) < 3:
				...
				print("exp: cluster_controller.py load 501 ouroboros_process.cfg")
				exit(1)
			...
			#If no UID parameter, the default is 0, the query all machines
			if uid < 0:
				uid = 0
