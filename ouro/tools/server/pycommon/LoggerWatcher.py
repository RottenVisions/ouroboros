...
class LoggerWatcher:
	...
	def deregisterFromLogger( self ):
		"""
		From logger to cancel the registration
		"""
		msg = Define.BytesIO()
	...
	def parseLog( self, stream ):
		"""
		The data stream from the decomposition of the log
	...
	def receiveLog( self, callbackFunc, loop = False ):
		...
		while True:
			...
			# Anyway, every once in a while send a heartbeat to avoid dropped calls
			self.sendActiveTick()
