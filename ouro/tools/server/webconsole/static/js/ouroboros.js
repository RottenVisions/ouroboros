 /*
 * Some similar to the ideas from the python utility functions
 * write by penghuawei
 */
...
Ouroboros = {
    ...
    // Received data is filtered, the vt100 terminal commands to filter out
    ...
    parsePyTickProfileData : function (data,cmd) {
       // The last is not the end of the data removed to continue the processing
        if ($.data.lastReceivedTickData)
        {
        ...
        for (var i = 0; i < ds.length; i++)
        {
            ...
            if ( cmd == 'pytickprofile')
            {
                ...
                if (!tickData)
                {
                    console.error("Ouroboros::parsePyTickProfileData(), unknown title data: %s", line);
                    continue;
                }
                ...
                {
                    console.error("Ouroboros::parsePyTickProfileData(), unknown data: %s", line);
                }
            }

            //Get the cprofile data
            if (cmd == 'cprofile')
            {
