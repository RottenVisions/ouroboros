...
clients_()
{
		...
		Loginapp::getSingleton().networkInterface().extTcpAddr().ip) == -1)
	{
		ERROR_MSG(fmt::format("HTTPCBHandler::bind({}): {}:{}\n",
			 ouro_strerror(), inet_ntoa((struct in_addr&)Loginapp::getSingleton().networkInterface().extTcpAddr().ip),
	...
	else
	{
		...
		if(code.size() > 0)
		{
			...
			if(type == 1)
			{
				...
				hellomessage = g_ouroSrvConfig.emailAtivationInfo_.backlink_hello_message;
			}
			else if(type == 2)
			{
				...
				hellomessage = g_ouroSrvConfig.emailResetPasswordInfo_.backlink_hello_message;
			}
			else if(type == 3)
			{
				std::string username;
				...
				hellomessage = g_ouroSrvConfig.emailBindInfo_.backlink_hello_message;
			}
...
void HTTPCBHandler::onAccountActivated(std::string& code, bool success)
{
	...
	for(; iter != clients_.end(); ++iter)
	{
		if(iter->second.code == code)
		{
			...
			if(success)
				message = g_ouroSrvConfig.emailAtivationInfo_.backlink_success_message;
			else
...
void HTTPCBHandler::onAccountBindedEmail(std::string& code, bool success)
{
	...
	for(; iter != clients_.end(); ++iter)
	{
		if(iter->second.code == code)
		{
			...
			if(success)
				message = g_ouroSrvConfig.emailBindInfo_.backlink_success_message;
			else
...
void HTTPCBHandler::onAccountResetPassword(std::string& code, bool success)
{
	...
	for(; iter != clients_.end(); ++iter)
	{
		if(iter->second.code == code)
		{
			...
			if(success)
				message = g_ouroSrvConfig.emailResetPasswordInfo_.backlink_success_message;
			else
