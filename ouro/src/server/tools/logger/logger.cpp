...
bool Logger::initializeEnd()
{
	...
	bool ret = pTelnetServer_->start(g_ouroSrvConfig.getLogger().telnet_passwd,
		g_ouroSrvConfig.getLogger().telnet_deflayer,
		g_ouroSrvConfig.getLogger().telnet_port);
...
//-------------------------------------------------------------------------------------
void Logger::writeLog(Network::Channel* pChannel, Ouroboros::MemoryStream& s)
{
	++g_secsNumlogs;
	++g_totalNumlogs;
    ...
    ouro_snprintf(timebuf, MAX_BUF, "%02d", (int)pLogItem->componentGroupOrder);
...
//-------------------------------------------------------------------------------------
void Logger::registerLogWatcher(Network::Channel* pChannel, Ouroboros::MemoryStream& s)
{
...
//-------------------------------------------------------------------------------------
void Logger::deregisterLogWatcher(Network::Channel* pChannel, Ouroboros::MemoryStream& s)
{
	logWatchers_.erase(pChannel->addr());
...
//-------------------------------------------------------------------------------------
void Logger::updateLogWatcherSetting(Network::Channel* pChannel, Ouroboros::MemoryStream& s)
{
	LogWatcher* pLogwatcher = &logWatchers_[pChannel->addr()];
