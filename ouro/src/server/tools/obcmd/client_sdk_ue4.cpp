...
static std::string ConflictTypePrefix = "OURO_";
static std::string namespaceNameBegin = "namespace Ouroboros\n{\n\n";
...
bool ClientSDKUE4::getArrayType(DataType* pDataType, std::string& outstr)
{
	if (pDataType->type() == DATA_TYPE_FIXEDARRAY)
	{
		...
		// if the element is an array again
		if (pFixedArrayType->getDataType()->type() == DATA_TYPE_FIXEDARRAY)
		{
			if (outstr.size() > 0)
				strutil::ouro_replace(outstr, pDataType->aliasName(), fmt::format("TArray<{}>", pFixedArrayType->getDataType()->aliasName()));
			else
		...
		{
			if (outstr.size() > 0)
				strutil::ouro_replace(outstr, pDataType->aliasName(), pFixedArrayType->getDataType()->aliasName());
			else
		...
		{
			if (outstr.size() > 0)
				strutil::ouro_replace(outstr, pDataType->aliasName(), typeToType(pFixedArrayType->getDataType()->getName()));
			else
...
bool ClientSDKUE4::writeServerErrorDescrsModuleBegin()
{
	...
	fileBody() += fmt::format("class OUROBOROSPLUGINS_API {}\n{{\npublic:\n", "ServerErrorDescrs");
...
bool ClientSDKUE4::writeEngineMessagesModuleMessage(Network::ExposedMessageInfo& messageInfos, COMPONENT_TYPE componentType)
{
	...
	fileBody() += fmt::format("class OUROBOROSPLUGINS_API Message_{} : public Message\n{{\npublic:\n", messageInfos.name);
	...
	if (messageInfos.argsTypes.size() == 0)
	{
		...
		if (componentType == CLIENT_TYPE)
		{
			...
			{
				fileBody() += fmt::format("\tOuroborosApp::getSingleton().{}(msgstream);\n", messageInfos.name);
			}
			else
			{
				fileBody() += fmt::format("\tOuroborosApp::getSingleton().{}();\n", messageInfos.name);
			}
		}
	}
	else
	{
		...
		for (int i = 0; i < (int)messageInfos.argsTypes.size(); ++i)
		{
			...
			OURO_ASSERT(nativetype != "FIXED_DICT" && nativetype != "ARRAY" && nativetype != "PYTHON" && nativetype != "ENTITYCALL");
...
bool ClientSDKUE4::writeEntityDefsModuleInitDefType(const DataType* pDataType)
{
	...
	{
		typeID = pDataType->id();
		OURO_ASSERT(typeID > datatype2id("OURO_DATATYPE2ID_MAX"));
	}
...
{
	fileBody() = headerBody;
	strutil::ouro_replace(fileBody(), "#REPLACE#", fmt::format("\t\n",
		fileBody()));
...
bool ClientSDKUE4::writeBaseEntityCallBegin(ScriptDefModule* pScriptDefModule)
{
	...
	newModuleName = std::string("EntityBaseEntityCall_") + std::string(pScriptDefModule->getName()) + moduleSuffix;
	fileBody() += fmt::format("class OUROBOROSPLUGINS_API {} : public EntityCall\n{{\npublic:\n", newModuleName);
...
bool ClientSDKUE4::createArrayChildClass(DataType* pRootDataType, DataType* pDataType, const std::string& className, const std::string& tabs, int numLayer)
{
	...
	fileBody() += fmt::format("{}class OUROBOROSPLUGINS_API DATATYPE_{} : public DATATYPE_BASE\n{}{{\n{}public:\n", tabs, className, tabs, tabs);
	...
	if (strcmp(pDataType->getName(), "FIXED_DICT") == 0)
	{
		...
		// If it is a non-anonymous array, the first level of parsing should be set directly to the category with the name
		// Otherwise set to system List category
		if (numLayer == 1)
		{
...
bool ClientSDKUE4::writeCustomDataType(const DataType* pDataType)
{
	...
	if (strcmp(pDataType->getName(), "FIXED_DICT") == 0)
	{
		fileBody() += fmt::format("\nclass OUROBOROSPLUGINS_API DATATYPE_{} : DATATYPE_BASE\n{{\npublic:\n", typeName);
		...
		// Create createFromStreamEx method
		{
			changeContextToHeader();
			fileBody() += fmt::format("\tvoid createFromStreamEx(MemoryStream& stream, {}& datas);\n", typeName);
...
{
	// If pDescr is None and is a client method, then you need to force useMethodDescrAlias to be true, otherwise the default will be false.
...
bool ClientSDKUE4::writeEntityModuleBegin(ScriptDefModule* pEntityScriptDefModule)
{
	...
	fileBody() = headerBody;
	strutil::ouro_replace(fileBody(), "#REPLACE#", fmt::format("\tPlease inherit this module, such as: (class {} : public {}{})\n",
	...
	if (pEntityScriptDefModule->isComponentModule())
	{
		fileBody() += fmt::format("class OUROBOROSPLUGINS_API {} : public EntityComponent\n{{\npublic:\n", newModuleName);

		// Write the entityCall attribute
	...
	else
	{
		...
		fileBody() += fmt::format("class OUROBOROSPLUGINS_API {} : public Entity\n{{\npublic:\n", newModuleName);
	...
	changeContextToHeader();
	// After the function returns, the subsequent process is to write the entity attribute:
	return true;
}
...
bool ClientSDKUE4::writeEntityProcessMessagesMethod(ScriptDefModule* pEntityScriptDefModule)
{
	...
	if (clientMethods.size() > 0)
	{
		...
		if (!pEntityScriptDefModule->isComponentModule())
		{
			...
			{
				fileBody() += fmt::format("\t\tOURO_ASSERT(false);\n");
			}
	...
	//The entity still has default properties even when the property is less than 0, and the component may have an empty switch compilation error because there is no property.
	...
	// Processing property callPropertysSetMethods
	changeContextToHeader();
	...
	ScriptDefModule* pCurrScriptDefModule, PropertyDescription* pPropertyDescription)
{
	if (std::string("ARRAY") == pPropertyDescription->getDataTypeName())
	{
		...
		Ouroboros::strutil::ouro_splits(fileBody(), " ", values);
		fileBody() = s + fileBody();
...
{
	// Need to parse for anonymous arrays, otherwise fill in the type name directly
