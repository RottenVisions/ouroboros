...
bool ClientSDK::writeEntityCall(ScriptDefModule* pScriptDefModule)
{
...

	{
		...
		for (; methodIter != scriptMethods.end(); ++methodIter)
		{
			...
			OURO_ASSERT((fHeaderPos != std::string::npos) || (fSourcePos != std::string::npos));
			...
			strutil::ouro_replace(headerfileBody_, "#REPLACE_FILLARGS1#", argsBody1);
			strutil::ouro_replace(headerfileBody_, "#REPLACE_FILLARGS2#", argsBody2);
			strutil::ouro_replace(sourcefileBody_, "#REPLACE_FILLARGS1#", argsBody1);
			strutil::ouro_replace(sourcefileBody_, "#REPLACE_FILLARGS2#", argsBody2);
	...
	for (; oiter != dataTypesOrders.end(); ++oiter)
	{
		...
		else if (pDataType->type() == DATA_TYPE_FIXEDARRAY)
		{
			...
			if (type == "ARRAY")
			{
				...
				strutil::ouro_replace(headerfileBody_, "#REPLACE#", newType);
				strutil::ouro_replace(sourcefileBody_, "#REPLACE#", newType);
			}
			else if (type == "FIXED_DICT")
			{
				strutil::ouro_replace(headerfileBody_, "#REPLACE#", itemTypeAliasName);
				strutil::ouro_replace(sourcefileBody_, "#REPLACE#", itemTypeAliasName);
			}
			else
			{
	...
	ScriptDefModule* pCurrScriptDefModule)
{
	...
	for (; methodIter != clientMethods.end(); ++methodIter)
	{
		...
		strutil::ouro_replace(headerfileBody_, "#REPLACE#", argsBody);
		strutil::ouro_replace(sourcefileBody_, "#REPLACE#", argsBody);
	}
