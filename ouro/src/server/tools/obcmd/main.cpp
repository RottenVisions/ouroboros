...
int process_make_client_sdk(int argc, char* argv[], const std::string clientType)
{
	...
	OBCMD app(dispatcher, networkInterface, g_componentType, g_componentID);
	...
	// If it detects that the zip file is set, then get the path from the zip file.
	if (compressionfile.size() > 0)
	{
		std::vector<std::string> tmpvec;
		Ouroboros::strutil::ouro_splits(compressionfile, ".", tmpvec);
		path = tmpvec[0];
...
{
	// Set the project environment variable of OURO_RES_PATH according to assetsType
	std::string res_path = getenv("OURO_RES_PATH") == NULL ? "" : getenv("OURO_RES_PATH");
	std::string root_path = getenv("OURO_ROOT") == NULL ? "" : getenv("OURO_ROOT");
	...
	setenv("OURO_RES_PATH", res_path.c_str(), 1);
...
int process_help(int argc, char* argv[])
{
	...
	printf("\tobcmd.exe --newassets=python --outpath=c:/xserver_assets\n");
