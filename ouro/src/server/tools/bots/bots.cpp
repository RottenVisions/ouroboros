...
pPyBots_(NULL),
clients_(),
reqCreateAndLoginTotalCount_(g_ouroSrvConfig.getBots().defaultAddBots_totalCount),
reqCreateAndLoginTickCount_(g_ouroSrvConfig.getBots().defaultAddBots_tickCount),
reqCreateAndLoginTickTime_(g_ouroSrvConfig.getBots().defaultAddBots_tickTime),
...
pTelnetServer_(NULL)
{
	...
	Ouroboros::Network::MessageHandlers::pMainMessageHandlers = &BotsInterface::messageHandlers;
...
bool Bots::initializeEnd()
{
	...
	if(!pTelnetServer_->start(g_ouroSrvConfig.getBots().telnet_passwd,
		g_ouroSrvConfig.getBots().telnet_deflayer,
		g_ouroSrvConfig.getBots().telnet_port))
	{
...
bool Bots::installPyModules()
{
	...
	PyObject *entryScriptFileName = PyUnicode_FromString(g_ouroSrvConfig.getBots().entryScriptFile);
	if(entryScriptFileName != NULL)
	{
		...
		if (PyErr_Occurred())
		{
			INFO_MSG(fmt::format("EntityApp::installPyModules: importing scripts/bots/{}.py...\n",
				g_ouroSrvConfig.getBots().entryScriptFile));
...
PyObject* Bots::__py_addBots(PyObject* self, PyObject* args)
{
	...
	if(PyTuple_Size(args) == 1)
	{
		...
		{
			PyErr_Format(PyExc_TypeError, "Ouroboros::addBots: args is error!");
			PyErr_PrintEx(0);
			return NULL;
		}
...
//-------------------------------------------------------------------------------------
void Bots::onExecScriptCommand(Network::Channel* pChannel, Ouroboros::MemoryStream& s)
{
	std::string cmd;
	s.readBlob(cmd);
...
//-------------------------------------------------------------------------------------
void Bots::onRemoteMethodCall(Network::Channel* pChannel, Ouroboros::MemoryStream& s)
{
	ClientObject* pClient = findClient(pChannel);
	if(pClient)
	{
...
//-------------------------------------------------------------------------------------
void Bots::onRemoteMethodCallOptimized(Network::Channel* pChannel, Ouroboros::MemoryStream& s)
{
	ClientObject* pClient = findClient(pChannel);
	if(pClient)
	{
...
//-------------------------------------------------------------------------------------
void Bots::startProfile(Network::Channel* pChannel, Ouroboros::MemoryStream& s)
{
