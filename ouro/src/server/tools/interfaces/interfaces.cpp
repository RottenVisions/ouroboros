...
bool Interfaces::initializeEnd()
{
	...
	bool ret = pTelnetServer_->start(g_ouroSrvConfig.getInterfaces().telnet_passwd,
		g_ouroSrvConfig.getInterfaces().telnet_deflayer,
		g_ouroSrvConfig.getInterfaces().telnet_port);
...
void Interfaces::onInstallPyModules()
{
	...
	for (int i = 0; i < SERVER_ERR_MAX; i++)
	{
		...
		{
			ERROR_MSG( fmt::format("Interfaces::onInstallPyModules: Unable to set Ouroboros.{}.\n", SERVER_ERR_STR[i]));
		}
	}
...
//-------------------------------------------------------------------------------------
void Interfaces::reqCreateAccount(Network::Channel* pChannel, Ouroboros::MemoryStream& s)
{
...
PyObject* Interfaces::__py_createAccountResponse(PyObject* self, PyObject* args)
{
	...
	Ouroboros::SERVER_ERROR_CODE errCode;
...
//-------------------------------------------------------------------------------------
void Interfaces::charge(Network::Channel* pChannel, Ouroboros::MemoryStream& s)
{
	OrdersCharge* pOrdersCharge = new OrdersCharge();

	pOrdersCharge->timeout = timestamp() + uint64(g_ouroSrvConfig.interfaces_orders_timeout_ * stampsPerSecond());
...
//-------------------------------------------------------------------------------------
void Interfaces::chargeResponse(std::string orderID, std::string extraDatas, Ouroboros::SERVER_ERROR_CODE errorCode)
{
