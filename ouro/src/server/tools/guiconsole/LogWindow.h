...
// Dialog Data
	...
	Ouroboros::uint32 getSelLogTypes();
	std::vector<Ouroboros::COMPONENT_TYPE> getSelComponents();

	void onReceiveRemoteLog(std::string str, bool fromServer = true);

	void onConnectionState(bool success, Ouroboros::Network::Address addr);

	void pullLogs(Ouroboros::Network::Address addr);
