...
void CProfileWindow::onReceivePyProfileData(Ouroboros::MemoryStream& s)
{
	std::string data;
	uint32 timinglen;
	s >> timinglen;
	...
	CString str;
	wchar_t* ws = Ouroboros::strutil::char2wchar(data.c_str());
	str = ws;
	str.Replace(L"\n", L"\r\n");
	free(ws);
...
void CProfileWindow::onReceiveCProfileData(Ouroboros::MemoryStream& s)
{
...
void CProfileWindow::onReceiveEventProfileData(Ouroboros::MemoryStream& s)
{
	uint32 timinglen;
	s >> timinglen;
	...
	while(size-- > 0)
	{
		...
		while(size1-- > 0)
		{
			...
			ws = Ouroboros::strutil::char2wchar(name.c_str());
			str.Format(L"%s\t\t\t\t\t%u\t%u\r\n", ws, count, eventSize);
			outstr += str;
			free(ws);
		}
...
void CProfileWindow::onReceiveNetworkProfileData(Ouroboros::MemoryStream& s)
{
	m_profileShowList.DeleteAllItems();
