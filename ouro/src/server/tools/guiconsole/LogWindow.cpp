...
void CLogWindow::onReceiveRemoteLog(std::string str, bool fromServer)
{
	...
	Ouroboros::strutil::ouro_split<wchar_t>(s.GetBuffer(0), L' ', logSplit);
...
void CLogWindow::onConnectionState(bool success, Ouroboros::Network::Address addr)
{
...
void CLogWindow::pullLogs(Ouroboros::Network::Address addr)
{
	CguiconsoleDlg* dlg = static_cast<CguiconsoleDlg*>(theApp.m_pMainWnd);
	...
	if(!pulling)
	{
		...
		char* cs = Ouroboros::strutil::wchar2char(apporder.GetBuffer(0));
		...
		cs = Ouroboros::strutil::wchar2char(apporder.GetBuffer(0));
		order = atoi(cs);
		free(cs);
		...
		cs = Ouroboros::strutil::wchar2char(date.GetBuffer(0));
		(*pBundle) << cs;
		free(cs);

		CString keystr;
		m_findEdit.GetWindowTextW(keystr);
		cs = Ouroboros::strutil::wchar2char(keystr.GetBuffer(0));
		(*pBundle) << cs;
		free(cs);

		int8 count = 0;
		std::vector<Ouroboros::COMPONENT_TYPE> vec = getSelComponents();
		count = vec.size();
		(*pBundle) << count;
		std::vector<Ouroboros::COMPONENT_TYPE>::iterator iter = vec.begin();
...
std::vector<Ouroboros::COMPONENT_TYPE> CLogWindow::getSelComponents()
{
	std::vector<Ouroboros::COMPONENT_TYPE> vec;
	for(int i=0; i<m_componentlist.GetCount(); i++)
	{
...
Ouroboros::uint32 CLogWindow::getSelLogTypes()
{
	Ouroboros::uint32 types = 0;
	for(int i=0; i<m_msgTypeList.GetCount(); i++)
	{
