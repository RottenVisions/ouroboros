...
BOOL CConnectRemoteMachineWindow::OnInitDialog()
{
	...
	for(; iter != m_historyCommand.end(); iter++)
	{
		if(i == 0)
		{
			...
			char* ip = Ouroboros::strutil::wchar2char(output.GetBuffer(0));
...
void CConnectRemoteMachineWindow::OnBnClickedOk()
{
	...
	Ouroboros::u_int16_t port = 0;
	CString sport;
	m_port.GetWindowTextW(sport);
	char* csport = Ouroboros::strutil::wchar2char(sport.GetBuffer(0));
	port = atoi(csport);
	...
	Ouroboros::Network::Address addr(address, htons(port));
	...
	while(true)
	{
		...
		Ouroboros::Network::Bundle bhandler;
		bhandler.newMessage(Ouroboros::MachineInterface::onFindInterfaceAddr);

		Ouroboros::MachineInterface::onFindInterfaceAddrArgs7::staticAddToBundle(bhandler, Ouroboros::getUserUID(), Ouroboros::getUsername(),
...
END:
	...
	Ouroboros::Network::EndPoint::reclaimPoolObject(endpoint);
	wchar_t* wcommand = Ouroboros::strutil::char2wchar(command.c_str());
	bool found = false;
	std::deque<CString>::iterator iter = m_historyCommand.begin();
	for(; iter != m_historyCommand.end(); iter++)
	{
...
void CConnectRemoteMachineWindow::saveIpMapping()
{
	...
	for (auto iter = m_ipMapping.begin(); iter != m_ipMapping.end(); iter = m_ipMapping.upper_bound(iter->first))
	{
		...
		host = iter->first;
		char* value = Ouroboros::strutil::wchar2char(host.GetBuffer(0));
