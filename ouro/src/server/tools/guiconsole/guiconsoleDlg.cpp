...
class ConsoleExecCommandCBMessageHandlerEx : public Ouroboros::ConsoleInterface::ConsoleExecCommandCBMessageHandler
{
public:
...
class ConsoleLogMessageHandlerEx : public Ouroboros::ConsoleInterface::ConsoleLogMessageHandler
{
public:
	virtual void handle(Network::Channel* pChannel, MemoryStream& s)
	{
...
class ConsoleWatcherCBMessageHandlerEx : public Ouroboros::ConsoleInterface::ConsoleWatcherCBMessageHandler
{
public:
	virtual void handle(Network::Channel* pChannel, MemoryStream& s)
	{
...
class ConsoleProfileHandlerEx : public Ouroboros::ConsoleInterface::ConsoleProfileHandler
{
public:
	virtual void handle(Network::Channel* pChannel, MemoryStream& s)
	{
...
public:
	...
	virtual bool process()
	{
		...
		while(true)
		{
			...
			{
				Ouroboros::sleep(10);
				nport = OURO_PORT_START + (rand() % 1000);
				continue;
			}
...
BOOL CguiconsoleDlg::OnInitDialog()
{
	...
	Ouroboros::ConsoleInterface::messageHandlers.add("Console::onExecScriptCommandCB", new Ouroboros::ConsoleInterface::ConsoleExecCommandCBMessageHandlerArgs1, NETWORK_VARIABLE_MESSAGE,
		new ConsoleExecCommandCBMessageHandlerEx);

	Ouroboros::ConsoleInterface::messageHandlers.add("Console::onReceiveRemoteLog", new Ouroboros::ConsoleInterface::ConsoleLogMessageHandlerArgsStream, NETWORK_VARIABLE_MESSAGE,
		new ConsoleLogMessageHandlerEx);

	Ouroboros::ConsoleInterface::messageHandlers.add("Console::onReceiveWatcherData", new Ouroboros::ConsoleInterface::ConsoleWatcherCBHandlerMessageArgsStream, NETWORK_VARIABLE_MESSAGE,
		new ConsoleWatcherCBMessageHandlerEx);

	Ouroboros::ConsoleInterface::messageHandlers.add("Console::onReceiveProfileData", new Ouroboros::ConsoleInterface::ConsoleProfileHandlerArgsStream, NETWORK_VARIABLE_MESSAGE,
		new ConsoleProfileHandlerEx);

	Ouroboros::Network::Bundle::ObjPool().pMutex(new Ouroboros::thread::ThreadMutex());
	Ouroboros::Network::TCPPacket::ObjPool().pMutex(new Ouroboros::thread::ThreadMutex());
	Ouroboros::Network::UDPPacket::ObjPool().pMutex(new Ouroboros::thread::ThreadMutex());
	Ouroboros::MemoryStream::ObjPool().pMutex(new Ouroboros::thread::ThreadMutex());
...
void CguiconsoleDlg::saveHistory()
{
	...
	{
		char key[256] = {0};
		ouro_snprintf(key, 256, "item%d", i++);
...
void CguiconsoleDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	case 1:
		...
		_networkInterface.processChannels(&Ouroboros::ConsoleInterface::messageHandlers);
		break;
	case 2:
		{
	...
	case 3:
		{
			...
			for(; iter != channels.end(); iter++)
			{
				...
				{
					COMMON_NETWORK_MESSAGE((Ouroboros::COMPONENT_TYPE)pChannel->proxyID(), (*pBundle), onAppActiveTick);
				}
				else
				{
...
bool CguiconsoleDlg::hasTreeComponent(Components::ComponentInfos& cinfos)
{
	...
	char sbuf[1024];
	ouro_snprintf(sbuf, 1024, "%s[%s]", COMPONENT_NAME_EX(cinfos.componentType), cinfos.pIntAddr->c_str());
	wchar_t* wbuf = Ouroboros::strutil::char2wchar(sbuf);
	CString s1 = wbuf;
	free(wbuf);
...
void CguiconsoleDlg::OnNMRClickTree1(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	CPoint point;
	GetCursorPos(&point); //Mouse position
...
int32 CguiconsoleDlg::getSelTreeItemUID()
{
	...
	char* buf = Ouroboros::strutil::wchar2char(s.GetBuffer(0));
	int32 uid = atoi(buf);
	free(buf);
...
Network::Address CguiconsoleDlg::getTreeItemAddr(HTREEITEM hItem)
{
	...
	Ouroboros::Network::Address addr(address, htons(atoi(sport.c_str())));
	return addr;
}
...
void CguiconsoleDlg::OnNMClickTree1(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	// Check box is selected to connect or disconnect
