...
BOOL CWatcherWindow::OnInitDialog()
{
...

	{
		...
		m_status.SetExtendedStyle(dwStyle);					//Set the extension style
	}
...

	{
		...
		dwStyle |= LVS_EX_GRIDLINES;						//Gridlines (only applicable with report style listctrl)
		//dwStyle |= LVS_EX_ONECLICKACTIVATE;
		m_statusShow.SetExtendedStyle(dwStyle);				//Set the extension style
	}
...
void CWatcherWindow::addItemShow(Ouroboros::WatcherObject* wo)
{
	int idx = 0;
	wchar_t* ws = Ouroboros::strutil::char2wchar(wo->str());
	CString s = ws;
	free(ws);

	ws = Ouroboros::strutil::char2wchar(wo->name());
	CString s1 = ws;
	free(ws);
...
void CWatcherWindow::addItem(Ouroboros::WatcherObject* wo)
{
	int idx = 0;
	wchar_t* ws = Ouroboros::strutil::char2wchar(wo->str());
	CString s = ws;
...
void CWatcherWindow::addPath(std::string rootpath, std::string path)
{
	...
	wchar_t* ws = Ouroboros::strutil::char2wchar(path.c_str());
	CString s = ws;
	free(ws);
...
HTREEITEM CWatcherWindow::findAndCreatePathItem(std::string path, HTREEITEM rootItem)
{
	...
	Ouroboros::strutil::ouro_split(path, '/', vec);

	wchar_t* ws = Ouroboros::strutil::char2wchar(vec[0].c_str());
	CString pathitem = ws;
	free(ws);
	...
	if(vec.size() > 1)
	{
		...
		else
		{
			...
			if(rootItem == NULL)
			{
				ws = Ouroboros::strutil::char2wchar(vec[1].c_str());
				pathitem = ws;
				free(ws);
				TV_INSERTSTRUCT tcitem;
				tcitem.hParent = rootItem;
...
void CWatcherWindow::onReceiveWatcherData(Ouroboros::MemoryStream& s)
{
	uint8 type = 0;
	s >> type;
	...
	if(type == 0)
	{
		Ouroboros::WatcherPaths watcherPaths;
		...
		while(s.length() > 0)
		{
			...
			Ouroboros::WATCHER_ID id = 0;
			s >> id;

			Ouroboros::WATCHER_VALUE_TYPE type;
			s >> type;

			Ouroboros::WatcherObject* wo = watcherPaths.addWatcherFromStream(path, name, id, type, &s);
			addHeader(name);
			addItem(wo);
		};
	}
...
std::string CWatcherWindow::getCurrSelPath()
{
	...
	char* str = Ouroboros::strutil::wchar2char(path.GetBuffer(0));
	std::string ret = str;
	free(str);
	return ret;
}
