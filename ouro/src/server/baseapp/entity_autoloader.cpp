	...
	for(int i=0; i<size; ++i)
	{
		...
		else
		{
			Baseapp::getSingleton().createEntityAnywhereFromDBID(EntityDef::findScriptModule(entityType)->getName(), dbid, NULL, 
				g_ouroSrvConfig.dbInterfaceIndex2dbInterfaceName(dbInterfaceIndex));
		}
	}
}
...
bool EntityAutoLoader::process()
{
	...
	if(entityTypes_.size() > 0)
	{
		if ((*entityTypes_.begin()).size() > 0)
		{
			...
			uint16 dbInterfaceIndex = (uint16)(g_ouroSrvConfig.getDBMgr().dbInterfaceInfos.size() - entityTypes_.size());

			if (!g_ouroSrvConfig.isPureDBInterfaceName(g_ouroSrvConfig.dbInterfaceIndex2dbInterfaceName(dbInterfaceIndex)))
			{
