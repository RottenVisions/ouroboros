...
ifndef INSTALL_DIR
ifeq ($(IS_COMMAND),1)
	OUTPUTDIR = $(OURO_ROOT)/ouro/bin/server/commands
else
...
# and Hybrid binaries are both placed in OURO_ROOT/tests/OURO_CONFIG not just
# the Hybrid builds.
ifdef INSTALL_ALL_CONFIGS
	OUTPUTDIR = $(INSTALL_DIR)/$(OURO_CONFIG)
else
...
ifeq ($(OURO_CONFIG), Hybrid) 
	OUTPUTDIR = $(INSTALL_DIR)
else # OURO_CONFIG == Hybrid

ifeq ($(OURO_CONFIG), Hybrid64) 
	OUTPUTDIR = $(INSTALL_DIR)
else # OURO_CONFIG == Hybrid64
	OUTPUTDIR = $(OURO_CONFIG)
endif # OURO_CONFIG == Hybrid64

endif # OURO_CONFIG == Hybrid
endif # INSTALL_DIR
...
MAKE_LIBS=1
ifndef OUTPUTDIR
	OUTPUTDIR = $(OURO_ROOT)/ouro/bin/server/$(COMPONENT)-extensions
endif # OUTPUTDIR
...
ZIP_DIR = $(OURO_ROOT)/ouro/src/lib/dependencies/zip
OURO_INCLUDES += -I$(ZIP_DIR)
ifeq ($(USE_ZIP),1)
LDLIBS += -lzip
CPPFLAGS += -DUSE_ZIP
endif
