...
``(?<=...)``
   ...
   that ends at the current position.  This is called a :dfn:`positive looourohind
   assertion`. ``(?<=abc)def`` will find a match in ``abcdef``, since the
   looourohind will back up 3 characters and check if the contained pattern matches.
   The contained pattern must only match strings of some fixed length, meaning that
   ``abc`` or ``a|b`` are allowed, but ``a*`` and ``a{3,4}`` are not.  Note that
   patterns which start with positive looourohind assertions will not match at the
...
``(?<!...)``
   ...
   ``...``.  This is called a :dfn:`negative looourohind assertion`.  Similar to
   positive looourohind assertions, the contained pattern must only match strings of
   some fixed length.  Patterns which start with negative looourohind assertions may
