...
"container" such as youâ€™d find in an actual gzip file. This snippet is just
...
Either of these can be used in place of a format string, to allow {}- or
$-formatting to be used to build the actual "message" part which appears in the
formatted log output in place of â€œ%(message)sâ€? or â€œ{message}â€? or â€œ$messageâ€?.
...
handler. So the only slightly unusual thing which might trip you up is that the
parentheses go around the format string and the arguments, not just the format
string. Thatâ€™s because the __ notation is just syntax sugar for a constructor
