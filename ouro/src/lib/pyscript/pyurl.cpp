...
void PyUrl::onHttpCallback(bool success, const Network::Http::Request& pRequest, const std::string& data)
{
	...
	OURO_ASSERT(pRequest.getUserargs());
...
PyObject* PyUrl::__py_urlopen(PyObject* self, PyObject* args)
{
	...
	else if (argCount == 3)
	{
		...
		if (PyDict_Check(pyobj))
		{
			...
			while (PyDict_Next(pyobj, &pos, &key, &value))
			{
				...
				{
					PyErr_Format(PyExc_AssertionError, "Ouroboros::urlopen: headers should be dict(keystr : valstr)!");
					PyErr_PrintEx(0);
					return NULL;
				}
		...
		{
			PyErr_Format(PyExc_AssertionError, "Ouroboros::urlopen: args3 is not postData(bytes) or callback(method)!");
			PyErr_PrintEx(0);
			return NULL;
		}
	}
	else if (argCount == 4)
	{
		...
		{
			PyErr_Format(PyExc_AssertionError, "Ouroboros::urlopen: POST data should be bytes!");
			PyErr_PrintEx(0);
			return NULL;
		}
	}
	...
	{
		PyErr_Format(PyExc_AssertionError, "Ouroboros::urlopen: invalid callback!");
		PyErr_PrintEx(0);
		return NULL;
	}
	...
	if (surl == NULL || ret == -1)
	{
		PyErr_Format(PyExc_AssertionError, "Ouroboros::urlopen: args error!");
		PyErr_PrintEx(0);
		return NULL;
	}
