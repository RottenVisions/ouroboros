...
static void initializeWatcher()
{
	...
	WATCH_OBJECT("db_querys/delete", &Ouroboros::watcher_delete);
	WATCH_OBJECT("db_querys/insert", &Ouroboros::watcher_insert);
	WATCH_OBJECT("db_querys/update", &Ouroboros::watcher_update);
	WATCH_OBJECT("db_querys/create", &Ouroboros::watcher_create);
	WATCH_OBJECT("db_querys/drop", &Ouroboros::watcher_drop);
	WATCH_OBJECT("db_querys/show", &Ouroboros::watcher_show);
	WATCH_OBJECT("db_querys/alter", &Ouroboros::watcher_alter);
	WATCH_OBJECT("db_querys/grant", &Ouroboros::watcher_grant);
}
...
bool DBInterfaceMysql::checkEnvironment()
{
	...
	if(pResult)
	{
		...
		while((arow = mysql_fetch_row(pResult)) != NULL)
		{
			...
			{
				uint64 size;
				Ouroboros::StringConv::str2value(size, v.c_str());
...
bool DBInterfaceMysql::dropEntityTableFromDB(const char* tableName)
{
	...
	ouro_snprintf(sql_str, SQL_BUF, "Drop table if exists %s;", tableName);
...
bool DBInterfaceMysql::dropEntityTableItemFromDB(const char* tableName, const char* tableItemName)
{
	...
	char sql_str[SQL_BUF];
	ouro_snprintf(sql_str, SQL_BUF, "alter table %s drop column %s;", tableName, tableItemName);
	return query(sql_str, strlen(sql_str));
}
