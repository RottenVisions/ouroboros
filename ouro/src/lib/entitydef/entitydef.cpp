...
// Generated automatically when the method is used for utype
...
// Generate utype automatically when new script module is generated
ENTITY_SCRIPT_UID g_scriptUtype = 1;

// Get the address of an entity function
...
void EntityDef::reload(bool fullReload)
{
	...
	if(fullReload)
	{
		...
		OURO_ASSERT(ret && "EntityDef::reload: finalise is error!");

		ret = initialize(EntityDef::__scriptBaseTypes, EntityDef::__loadComponentType);
		OURO_ASSERT(ret && "EntityDef::reload: initialize is error!");
	}
	else
	{
						   ...
						   COMPONENT_TYPE loadComponentType)
{
	...
	XML_FOR_BEGIN(node)
	{
		...
		// Loading definition in def file
		...
		// Attempt to load detailLevel data in the primary entity file
							...
							ScriptDefModule* pScriptModule)
{
	...
	// Attempt to load detailLevel data
	...
	// Attempt to load VolatileInfo data
	...
	ScriptDefModule* pScriptModule)
{
	...
	XML_FOR_BEGIN(implementsNode)
	{
		...
		uint32						databaseLength = 0;			// The length of this attribute in the database
		...
		// Find whether there is this module, if there is description has loaded the relevant description, there is no need to load again
									  ...
									  ScriptDefModule* pScriptModule)
{
	...
	// Loading client method description
								   ...
								   ScriptDefModule* pScriptModule)
{
	if(defMethodNode)
	{
		XML_FOR_BEGIN(defMethodNode)
		{
			...
			// If utype is not set in the configuration, then
...
PyObject* EntityDef::loadScriptModule(std::string moduleName)
{
	...
	// Check if the path of the module is under the OURO script directory. Prevent the system module from being mistakenly imported because of a conflict between the name of the user and the name of the python module.
	if (pyModule)
	{
...
bool EntityDef::loadAllComponentScriptModules(std::string entitiesPath, std::vector<PyTypeObject*>& scriptBaseTypes)
{
	...
	// All component class names that need to load the script
	...
	// Loading Entity Component Scripts
	...
	for (; comp_iter != componentTypes.end(); ++comp_iter)
	{
		...
		if (pyModule == NULL)
		{
			// If it is currently a tool, such as obcmd, the script cannot be loaded, and if a module does not have a client, it is deleted.
...
ERASE_PROPERTYS:
			...
			// Must be set here, setting before will cause isLoadScriptModule to fail, and no error output
									...
									std::vector<PyTypeObject*>& scriptBaseTypes)
{
	...
	XML_FOR_BEGIN(node)
	{
		...
		// Find whether the entity has corresponding entity components that need to be implemented in the process, and if there is not, an error is displayed
		const ScriptDefModule::COMPONENTDESCRIPTION_MAP& componentDescrs = pScriptModule->getComponentDescrs();
		ScriptDefModule::COMPONENTDESCRIPTION_MAP::const_iterator comp_iter = componentDescrs.begin();
		for (; comp_iter != componentDescrs.end(); ++comp_iter)
		{
			...
			// Whether to load this module, if there is a need to load and no module is currently there is an error
