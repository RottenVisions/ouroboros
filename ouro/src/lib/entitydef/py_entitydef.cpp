...
namespace Ouroboros{ namespace script{ namespace entitydef {
...
bool DefContext::addChildContext(DefContext& defContext)
{
	...
	for (; iter != pContexts->end(); ++iter)
	{
		...
		{
			// This can happen when there are multiple assemblyContexts
			// Do not do anything
...
static bool assemblyContexts(bool notfoundModuleError = false)
{
	...
	// Try to fill the parent class information into the derived class
...
static bool registerDefContext(DefContext& defContext)
{
	...
	if (defContext.type == DefContext::DC_TYPE_PROPERTY)
	{
		...
		// Check if the scope belongs to the process
		bool flagsGood = true;
	...
	else if (defContext.type == DefContext::DC_TYPE_CLIENT_METHOD)
	{
		...
		// Since the declaration name of the client method may be consistent with the server method, the client method needs to be temporarily named as individual name.
	...
	if (iter != DefContext::allScriptDefContextMaps.end())
	{
		...
		{
			// If it is a script part of a different process, then a merge registration is required
				...
				iter->second.type == defContext.type)
			{
				...
				for (; bciter != defContext.baseClasses.end(); ++bciter)
				{
					...
					OURO_ASSERT(defContext.base_methods.size() == 0 && 
				...
				// The script object is forced to be set to the object of the current process
...
{	\
	...
	{	\
		...
		{	\
						/*Prevent path inconsistency caused by different systems and eliminate system related paths*/		\
			OUT = PyUnicode_AsUTF8AndSize(pyFile, NULL);	\
			strutil::ouro_replace(OUT, "\\\\", "/");	\
			strutil::ouro_replace(OUT, "\\", "/");	\
			strutil::ouro_replace(OUT, "//", "/");	\
			std::string ouro_root = Resmgr::getSingleton().getPyUserScriptsPath();	\
			strutil::ouro_replace(ouro_root, "\\\\", "/");	\
			strutil::ouro_replace(ouro_root, "\\", "/");	\
			strutil::ouro_replace(ouro_root, "/", "/");	\
			strutil::ouro_replace(OUT, ouro_root, "");	\
			Py_DECREF(pyFile);	\
		}	\
	}	\
	else	\
	...
	{	\
		...
		{	\
						/*Add the uplink number to avoid multiple definitions in the same file*/		\
...
static PyObject* __py_def_parse(PyObject *self, PyObject* args)
{
	...
	else if (defContext.optionName == "property" || defContext.optionName == "fixed_item")
	{
		...
		else
		{
			...
			// If it is a data type definition, the default should be stored
			// fixed_item will only be used when defining fixed_dict
		...
		// For properties, we need to get the return value as the default
		...
		if (pyRet != Py_None)
		{
			...
			// Verify that this string can be restored to an object
	...
	else if (defContext.optionName == "component")
	{
		if (PyFunction_Check(pyFunc))
		{
			...
			// If it is a component property, the default should not be stored
	...
	if (!defContext.isModuleScope)
	{
		...
		if (moduleQualname)
			strutil::ouro_splits(moduleQualname, ".", outs);
		...
		if (pyGetfullargspec)
		{
			...
			else
			{
				...
				while (PyDict_Next(pyGetMethodAnnotationsResult, &pos, &key, &value)) {
					...
					// If it is EntityDef.Array then it may be a tuple here, see __py_array
	...
	else
	{
		...
		if (basesSize == 0)
		{
			PyErr_Format(PyExc_AssertionError, "EntityDef.%s: \'%s\' does not inherit the Ouroboros.Entity class!\n", defContext.optionName.c_str(), moduleQualname);
			Py_XDECREF(pyBases);
			PY_RETURN_ERROR;
		}
		...
		for (Py_ssize_t i = 0; i < basesSize; ++i)
		{
			...
			else if (parentClass == "Proxy")
			{
				if (defContext.componentType != BASEAPP_TYPE)
				{
					PyErr_Format(PyExc_AssertionError, "EntityDef.%s: \'%s\' Only BASE can inherit Ouroboros.Proxy!\n", defContext.optionName.c_str(), moduleQualname);
					PY_RETURN_ERROR;
				}
	...
	else if (defContext.optionName == "anonymous_fixed_array")
	{
		...
		// Prevent the number of lines of the two source files consistent, the bottom layer thinks that repeated additions
...
static PyObject* __py_def_rename(PyObject* self, PyObject* args, PyObject* kwargs)
{
	...
	// Similar to this definition EntityDef.rename(ENTITY_ID=EntityDef.INT32)
	if (kwargs)
	{
		PyObject *key, *value;
		Py_ssize_t pos = 0;
...
static PyObject* __py_def_fixed_array(PyObject* self, PyObject* args, PyObject* kwargs)
{
	...
	// Similar to this definition EntityDef.fixed_array(XXArray=EntityDef.INT32)
	if (kwargs)
	{
		PyObject *key, *value;
		Py_ssize_t pos = 0;
...
static bool loadAllScriptForComponentType(COMPONENT_TYPE loadComponentType)
{
	...
	// Priority execution of the entry script
	...
	for (; iter != results.end(); ++iter)
	{
		...
		strutil::ouro_replace(packetPath, rootPath, "");
		while (packetPath.size() > 0 && (packetPath[0] == '/' || packetPath[0] == '\\')) packetPath.erase(0, 1);
		strutil::ouro_replace(packetPath, "/", ".");
		strutil::ouro_replace(packetPath, "\\", ".");
...
static bool execPython(COMPONENT_TYPE componentType)
{
	...
	strutil::ouro_replace(pyPaths.second, L";", L":");
#endif
...

	{
		...
		while (PyDict_Next(modulesOld, &pos, &key, &value))
		{
			...
			if (std::string(typeName) == "Ouroboros")
				continue;
			...
			if (pyDoc)
			{
				...
				if (doc && std::string(doc).find("Ouroboros") != std::string::npos)
	...
	// Py_EndInterpreter cannot be used here, which will cause the destruction of modules such as Math and Def.
...
static bool updateScript(DefContext& defContext)
{
	...
	else
	{
		...
		if (typeNames.size() > 0)
		{
			ERROR_MSG(fmt::format("PyEntityDef::registerEntityDefs: registerEntityDefs {} is not derived from Ouroboros.[{}]\n",
...
static bool registerDefTypes()
{
	...
	{
		// Because the type of registration has a sequential dependency, here to sort them and then follow up
		DefContext::DEF_CONTEXT_MAP::iterator iter = DefContext::allScriptDefContextMaps.begin();
		for (; iter != DefContext::allScriptDefContextMaps.end(); ++iter)
		{
			DefContext& defContext = iter->second;
...
static bool registerDefMethods(ScriptDefModule* pScriptModule, DefContext& defContext)
{
	...
	for (; iter != methods.end(); ++iter)
	{
		...
		// Check if the first parameter is exposed callerID
		if (defMethodContext.argsvecs.size() > 0)
		{
			...
			// If the callerID parameter of the exposed method is ignored
	...
	// In addition to these processes, other processes do not need to detect the correctness of the script based on the method.
...
static bool registerDefPropertys(ScriptDefModule* pScriptModule, DefContext& defContext)
{
	...
	for (; iter != propertys.end(); ++iter)
	{
		...
		uint32 databaseLength = defPropContext.databaseLength; // the length of this property in the database
		...
		if (!dataType)
		{
			...
			// component is placed in the component function to handle
...
static bool registerDefComponents(ScriptDefModule* pScriptModule, DefContext& defContext)
{
	...
	for (; iter != components.end(); ++iter)
	{
		...
		// Except for these processes, other processes do not need to access the script
		if (g_componentType == BASEAPP_TYPE || g_componentType == CELLAPP_TYPE || g_componentType == BOTS_TYPE || g_componentType == CLIENT_TYPE)
		{
			// If it is a bots type, you need to set the script class to the class of the program environment.
			// Note: If it is CLIENT_TYPE, you can only use the def file mode or put the definition in a common py, because the schema definition is on the server code, and the client environment has no server code.
		...
		// Try to load the detailLevelInfo data
...
static bool registerEntityDef(ScriptDefModule* pScriptModule, DefContext& defContext)
{
	...
	// Register component descriptions and add their methods and properties to the module
	...
	// Try to register the detailLevelInfo data
	...
	// Try to register volatileInfo data
...
static bool registerEntityDefs()
{
	...
	for (; iter != DefContext::allScriptDefContextMaps.end(); ++iter)
	{
		...
		// Decide whether the entity has a client part depending on whether the client property or method is included
		// hasClient=True can also force the specified client part
...
bool initialize()
{
	...
	OURO_ASSERT(pyDefModuleName.size() > 0);
	PyObject *entitydefModule = PyImport_AddModule(pyDefModuleName.c_str());
	...
	OURO_ASSERT(baseTypeNames.size() < 64);

	for (size_t idx = 0; idx < baseTypeNames.size(); idx++)
	{
		memset(allBaseTypeNames[idx], 0, MAX_BUF);
		ouro_snprintf(allBaseTypeNames[idx], MAX_BUF, "%s", baseTypeNames[idx].c_str());
