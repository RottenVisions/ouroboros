...
bool ServerApp::initializeWatcher()
{
	...
	WATCH_OBJECT("uid", &Ouroboros::getUserUID);
	WATCH_OBJECT("username", &Ouroboros::getUsername);
...
void ServerApp::hello(Network::Channel* pChannel, MemoryStream& s)
{
	...
	if (encryptedKey.size() > 3 && encryptedKey.size() <= 65535)
	{
		for (int i = 0; i < (int)encryptedKey.size(); ++i)
		{
			...
			ouro_snprintf(buf, MAX_BUF / 2, "%02hhX ", (unsigned char)encryptedKey.data()[i]);
...
//-------------------------------------------------------------------------------------
void ServerApp::startProfile(Network::Channel* pChannel, Ouroboros::MemoryStream& s)
{
	if(pChannel->isExternal())
		return;
