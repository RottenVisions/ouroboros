...
namespace Ouroboros{
OURO_SINGLETON_INIT(ServerConfig);
...
bool ServerConfig::loadConfig(std::string fileName)
{
	...
	if(rootNode != NULL)
	{
		...
		if(childnode)
		{
			thread_timeout_ = float(OURO_MAX(1.0, xml->getValFloat(childnode)));
		}

		childnode = xml->enterNode(rootNode, "init_create");
		if(childnode)
		{
			thread_init_create_ = OURO_MAX(1, xml->getValInt(childnode));
		}

		childnode = xml->enterNode(rootNode, "pre_create");
		if(childnode)
		{
			thread_pre_create_ = OURO_MAX(1, xml->getValInt(childnode));
		}

		childnode = xml->enterNode(rootNode, "max_create");
		if(childnode)
		{
			thread_max_create_ = OURO_MAX(1, xml->getValInt(childnode));
		}
	}
	...
	if(rootNode != NULL)
	{
		...
		if(childnode)
		{
			...
			if(childnode1)
			{
				channelCommon_.channelInternalTimeout = OURO_MAX(0.f, float(xml->getValFloat(childnode1)));
			...
			if(childnode)
			{
				channelCommon_.channelExternalTimeout = OURO_MAX(0.f, float(xml->getValFloat(childnode1)));
		...
		if(childnode)
		{
			TiXmlNode* childnode1 = xml->enterNode(childnode, "internal");
			if(childnode1)
				channelCommon_.intReadBufferSize = OURO_MAX(0, xml->getValInt(childnode1));

			childnode1 = xml->enterNode(childnode, "external");
			if(childnode1)
				channelCommon_.extReadBufferSize = OURO_MAX(0, xml->getValInt(childnode1));
		}
		...
		if(childnode)
		{
			TiXmlNode* childnode1 = xml->enterNode(childnode, "internal");
			if(childnode1)
				channelCommon_.intWriteBufferSize = OURO_MAX(0, xml->getValInt(childnode1));

			childnode1 = xml->enterNode(childnode, "external");
			if(childnode1)
				channelCommon_.extWriteBufferSize = OURO_MAX(0, xml->getValInt(childnode1));
		}
		...
		if(childnode)
		{
			...
			if(sendNode)
			{
				...
				if(childnode1)
				{
					...
					if(childnode2)
						Network::g_intSendWindowMessagesOverflow = OURO_MAX(0, xml->getValInt(childnode2));

					childnode2 = xml->enterNode(childnode1, "external");
					if(childnode2)
						Network::g_extSendWindowMessagesOverflow = OURO_MAX(0, xml->getValInt(childnode2));

					childnode2 = xml->enterNode(childnode1, "critical");
					if(childnode2)
						Network::g_sendWindowMessagesOverflowCritical = OURO_MAX(0, xml->getValInt(childnode2));
				}
				...
				if (childnode1)
				{
					TiXmlNode* childnode2 = xml->enterNode(childnode1, "internal");
					if (childnode2)
						Network::g_intSentWindowBytesOverflow = OURO_MAX(0, xml->getValInt(childnode2));

					childnode2 = xml->enterNode(childnode1, "external");
					if (childnode2)
						Network::g_extSentWindowBytesOverflow = OURO_MAX(0, xml->getValInt(childnode2));
				}
			}
			...
			if(recvNode)
			{
				...
				if(childnode1)
				{
					TiXmlNode* childnode2 = xml->enterNode(childnode1, "internal");
					if(childnode2)
						Network::g_intReceiveWindowMessagesOverflow = OURO_MAX(0, xml->getValInt(childnode2));

					childnode2 = xml->enterNode(childnode1, "external");
					if(childnode2)
						Network::g_extReceiveWindowMessagesOverflow = OURO_MAX(0, xml->getValInt(childnode2));

					childnode2 = xml->enterNode(childnode1, "critical");
					if(childnode2)
						Network::g_receiveWindowMessagesOverflowCritical = OURO_MAX(0, xml->getValInt(childnode2));
				}
		...
		if(rudpChildnode)
		{
			...
			if (childnode)
			{
				TiXmlNode* childnode1 = xml->enterNode(childnode, "internal");
				if (childnode1)
					Network::g_rudp_intReadPacketsQueueSize = OURO_MAX(0, xml->getValInt(childnode1));

				childnode1 = xml->enterNode(childnode, "external");
				if (childnode1)
					Network::g_rudp_extReadPacketsQueueSize = OURO_MAX(0, xml->getValInt(childnode1));
			}
			...
			if (childnode)
			{
				TiXmlNode* childnode1 = xml->enterNode(childnode, "internal");
				if (childnode1)
					Network::g_rudp_intWritePacketsQueueSize = OURO_MAX(0, xml->getValInt(childnode1));

				childnode1 = xml->enterNode(childnode, "external");
				if (childnode1)
					Network::g_rudp_extWritePacketsQueueSize = OURO_MAX(0, xml->getValInt(childnode1));
			}

			childnode = xml->enterNode(rudpChildnode, "tickInterval");
			if (childnode)
			{
				Network::g_rudp_tickInterval = OURO_MAX(0, xml->getValInt(childnode));
			}

			childnode = xml->enterNode(rudpChildnode, "minRTO");
			if (childnode)
			{
				Network::g_rudp_minRTO = OURO_MAX(0, xml->getValInt(childnode));
			}

			childnode = xml->enterNode(rudpChildnode, "mtu");
			if (childnode)
			{
				Network::g_rudp_mtu = OURO_MAX(0, xml->getValInt(childnode));
			}

			childnode = xml->enterNode(rudpChildnode, "missAcksResend");
			if (childnode)
			{
				Network::g_rudp_missAcksResend = OURO_MAX(0, xml->getValInt(childnode));
			}
		...
		if(databaseInterfacesNode != NULL)
		{
			if (databaseInterfacesNode->FirstChild() != NULL)
			{
				do
				{
					...
					// The default library is not allowed to be a clean library, the engine needs to create an entity table
					if (name == "default")
					...
					if (pDBInfo == &dbinfo)
					{
						// Check that you cannot use the same database with the same table in different interfaces
						...
						{
							if (ouro_stricmp((*dbinfo_iter).db_ip, dbinfo.db_ip) == 0 && 
								ouro_stricmp((*dbinfo_iter).db_type, dbinfo.db_type) == 0 &&
						...
						if (fileName == "server/ouroboros_defaults.xml" && !missingFields.empty())
						{
							std::vector<std::string>::const_iterator iter = missingFields.begin();
							for (; iter != missingFields.end(); iter++)
							{
								ERROR_MSG(fmt::format("ServerConfig::loadConfig: ouroboros_defaults.xml error, databaseInterface({}) missing filed:{}.\n", name, *iter));
							}
...
{
	// If it is less than 64, it means that it is still a plain text password.
	if(emailServerInfo_.password.size() < 64)
	{
		WARNING_MSG(fmt::format("ServerConfig::loadConfig: email password(email_service.xml) is not encrypted!\nplease use password(rsa):\n{}\n"
			, OUROKey::getSingleton().encrypt(emailServerInfo_.password)));
	}
	else
	{
		std::string out = OUROKey::getSingleton().decrypt(emailServerInfo_.password);
		if(out.size() == 0)
		{
							   ...
							   const Network::Address& internalTcpAddr, const Network::Address& externalTcpAddr, const Network::Address& externalUdpAddr)
{
	...
	if(componentType == CELLAPP_TYPE)
	{
		...
		if (info.ids_criticallyLowSize > getDBMgr().ids_increasing_range / 2)
		{
			...
			ERROR_MSG(fmt::format("ouroboros[_defs].xml->cellapp->ids->criticallyLowSize > dbmgr->ids->increasing_range / 2, Force adjustment to criticallyLowSize({})\n", 
	...
	else if (componentType == BASEAPP_TYPE)
	{
		...
		{
			info.ids_criticallyLowSize = getDBMgr().ids_increasing_range / 2;
			ERROR_MSG(fmt::format("ouroboros[_defs].xml->baseapp->ids->criticallyLowSize > dbmgr->ids->increasing_range / 2, Force adjustment to criticallyLowSize({})\n",
				info.ids_criticallyLowSize));
		}
	...
	else if (componentType == DBMGR_TYPE)
	{
		...
		if (info.ids_increasing_range < 500)
		{
			...
			ERROR_MSG(fmt::format("ouroboros[_defs].xml-> dbmgr->ids->increasing_range too small, Force adjustment to ids_increasing_range({})\n",
