...
{
	/*
	When data is available, table data is generated
	ouro_entitylog:dbid:entityType = hashes(entityID, ip, port, componentID, logger)
	*/
					...
					COMPONENT_ID componentID, ENTITY_ID entityID, ENTITY_SCRIPT_UID entityType)
{
	...
	ouro_entitylog:dbid:entityType = hashes(entityID, ip, port, componentID, logger)
	*/
	std::string sqlstr = fmt::format("HSET " OURO_TABLE_PERFIX "_entitylog:{}:{} entityID {} ip {} port {} componentID {} logger {}",
...
bool OUROEntityLogTableRedis::queryEntity(DBInterface * pdbi, DBID dbid, EntityLog& entitylog, ENTITY_SCRIPT_UID entityType)
{
	...
	*/
	...
	static_cast<DBInterfaceRedis*>(pdbi)->query(fmt::format("HMGET " OURO_TABLE_PERFIX "_entitylog:{}:{} entityID ip port componentID logger",
...
//-------------------------------------------------------------------------------------
bool OUROServerLogTableRedis::clearTimeoutLogs(DBInterface * pdbi, const std::vector<COMPONENT_ID>& cids)
{
	return true;
}
...
bool OUROEmailVerificationTableRedis::activateAccount(DBInterface * pdbi, const std::string& code, ACCOUNT_INFOS& info)
{
	...
	*/
	...
	{
		ERROR_MSG(fmt::format("OUROEmailVerificationTableRedis::activateAccount({}): set deadline is error({})!\n",
	...
	{
		ERROR_MSG(fmt::format("OUROEmailVerificationTableRedis::activateAccount({}): update password is error({})!\n",
				code, pdbi->getstrerror()));
	...
	*/
	...
	{
		ERROR_MSG(fmt::format("OUROEmailVerificationTableRedis::activateAccount({}): update " OURO_TABLE_PERFIX "_accountinfos is error({})!\n",
				code, pdbi->getstrerror()));
