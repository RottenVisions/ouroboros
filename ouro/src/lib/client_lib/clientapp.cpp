...
state_(C_STATE_INIT)
{
	...
	Ouroboros::Network::MessageHandlers::pMainMessageHandlers = &ClientInterface::messageHandlers;
...
bool ClientApp::initializeBegin()
{
	...
	ProfileVal::setWarningPeriod(stampsPerSecond() / g_ouroConfig.gameUpdateHertz());
...
bool ClientApp::initializeEnd()
{
	...
	if(g_ouroConfig.useLastAccountName())
	{
		EventData_LastAccountInfo eventdata;
		eventdata.name = g_ouroConfig.accountName();
...
bool ClientApp::installEntityDef()
{
	...
	// Register some interfaces to ouroboros
...
//-------------------------------------------------------------------------------------
PyObject* ClientApp::__py_ouroOpen(PyObject* self, PyObject* args)
{
	int argCount = PyTuple_Size(args);
	if (argCount != 2)
	{
