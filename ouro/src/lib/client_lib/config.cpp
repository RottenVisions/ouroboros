...
bool Config::loadConfig(std::string fileName)
{
	...
	if(rootNode != NULL)
	{
		...
		if(childnode)
		{
			...
			if(childnode1)
			{
				channelInternalTimeout_ = OURO_MAX(0.f, float(xml->getValFloat(childnode1)));
			...
			if(childnode)
			{
				channelExternalTimeout_ = OURO_MAX(0.f, float(xml->getValFloat(childnode1)));
