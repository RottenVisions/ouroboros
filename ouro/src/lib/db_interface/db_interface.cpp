...
bool DBUtil::initialize()
{
	...
	{
		ERROR_MSG(fmt::format("DBUtil::initialize: not found database interface! (ouroboros[_defs].xml->dbmgr->databaseInterfaces)\n"));
		return false;
	}
		...
		if ((*dbinfo_iter).db_passwordEncrypt)
		{
			...
			if (strlen((*dbinfo_iter).db_password) < 64)
			{
				WARNING_MSG(fmt::format("DBUtil::initialize: db({}) password is not encrypted!\nplease use password(rsa):\n{}\n",
					(*dbinfo_iter).name, OUROKey::getSingleton().encrypt((*dbinfo_iter).db_password)));
			}
			else
			{
				std::string out = OUROKey::getSingleton().decrypt((*dbinfo_iter).db_password);

				if (out.size() == 0)
					return false;

				ouro_snprintf((*dbinfo_iter).db_password, MAX_BUF, "%s", out.c_str());
			}
		}
	}
...
DBInterface* DBUtil::createInterface(const std::string& name, bool showinfo)
{
	...
	ouro_snprintf(dbinterface->db_username_, MAX_BUF, "%s", pDBInfo->db_username);
	dbinterface->db_numConnections_ = pDBInfo->db_numConnections;
	ouro_snprintf(dbinterface->db_password_, MAX_BUF * 10, "%s", pDBInfo->db_password);
...
{
	DBInterfaceInfo* pDBInfo = g_ouroSrvConfig.dbInterface(pdbi->name());
	if (!pDBInfo)
	{
