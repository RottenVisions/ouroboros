...
void Resmgr::updatePaths()
{
	...
	// Windows users can't split the colon, it might split the drive letter.
#if OURO_PLATFORM != PLATFORM_WIN32
	if(respaths_.size() < 2)
	{
		respaths_.clear();
...
bool Resmgr::initialize()
{
	...
	// Get the engine environment configuration
...
std::string Resmgr::getPySysResPath()
{
	...
	if(respath == "")
	{
		...
		Ouroboros::strutil::ouro_splits(respath, "server/ouroboros_defaults.xml", tmpvec);
...
std::string Resmgr::getPyUserResPath()
{
	...
	if(respath == "")
	{
		...
		std::vector<std::string> tmpvec;
		Ouroboros::strutil::ouro_splits(respath, "server/ouroboros.xml", tmpvec);
