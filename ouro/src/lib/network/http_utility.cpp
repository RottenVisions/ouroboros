	...
	url_()
{
	...
	OURO_ASSERT(sizeof(error_) >= CURL_ERROR_SIZE);
...
Request::Status Request::setURL(const std::string& url)
{
	CURLcode curlCode = CURLE_OK;
		...
		if (!setVerifySSL_/*if not set, automatically set a default value*/ && url.substr(0, 6) == "https:")
	{
...
{
	if(headers_)
		curl_slist_append((curl_slist *)headers_, strutil::ouro_trim(header).c_str());
	else
		headers_ = curl_slist_append(NULL, strutil::ouro_trim(header).c_str());
...
Request::Status Request::setHeader(const std::vector<std::string>& headers)
{
	...
	{
		if (headers_)
			curl_slist_append((curl_slist *)headers_, strutil::ouro_trim((*iter)).c_str());
		else
			headers_ = curl_slist_append(NULL, strutil::ouro_trim((*iter)).c_str());
...
Request::Status Request::setHeader(const std::map<std::string, std::string>& headers)
{
	...
	for (; iter != headers.end(); ++iter)
	{
		std::string header = strutil::ouro_trim(iter->first);
		header += ": ";
		header += strutil::ouro_trim(iter->second);
