...
const char* MessageHandler::c_str()
{
	...
	ouro_snprintf(buf, MAX_BUF, "id:%u, len:%d", msgID, msgLen);
	return buf;
}
...
bool MessageHandlers::initializeWatcher()
{
	...
	for(; iter != msgHandlers_.end(); ++iter)
	{
		...
		{
			sname = name() + "::" + sname;
			strutil::ouro_replace(sname, "Interface::", "::");
		}

		char buf[MAX_BUF * 2];
		ouro_snprintf(buf, MAX_BUF * 2, "network/messages/%s/id", sname.c_str());
		WATCH_OBJECT(buf, iter->second->msgID);

		ouro_snprintf(buf, MAX_BUF * 2, "network/messages/%s/len", sname.c_str());
		WATCH_OBJECT(buf, iter->second->msgLen);

		ouro_snprintf(buf, MAX_BUF * 2, "network/messages/%s/sentSize", sname.c_str());
		WATCH_OBJECT(buf, iter->second, &MessageHandler::sendsize);

		ouro_snprintf(buf, MAX_BUF * 2, "network/messages/%s/sentCount", sname.c_str());
		WATCH_OBJECT(buf, iter->second, &MessageHandler::sendcount);

		ouro_snprintf(buf, MAX_BUF * 2, "network/messages/%s/sentAvgSize", sname.c_str());
		WATCH_OBJECT(buf, iter->second, &MessageHandler::sendavgsize);

		ouro_snprintf(buf, MAX_BUF * 2, "network/messages/%s/recvSize", sname.c_str());
		WATCH_OBJECT(buf, iter->second, &MessageHandler::recvsize);

		ouro_snprintf(buf, MAX_BUF * 2, "network/messages/%s/recvCount", sname.c_str());
		WATCH_OBJECT(buf, iter->second, &MessageHandler::recvsize);

		ouro_snprintf(buf, MAX_BUF * 2, "network/messages/%s/recvAvgSize", sname.c_str());
