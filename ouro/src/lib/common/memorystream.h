...
public:
    ...
    void print_storage() const
    {
		...
		{
			ouro_snprintf(buf, 1024, "%u ", read<uint8>(i));
			fbuffer += buf;
		}
    ...
    void textlike() const
    {
		...
		for (size_t i = rpos(); i < wpos(); ++i)
		{
			ouro_snprintf(buf, 1024, "%c", read<uint8>(i));
			fbuffer += buf;
		}
    ...
    void hexlike() const
    {
		...
		for (size_t idx = rpos(); idx < wpos(); ++idx)
        {
            ...
            if ((i == (j * 8)) && ((i != (k * 16))))
            {
                ...
                {
					ouro_snprintf(buf, 1024, "| 0%X ", read<uint8>(idx));
					fbuffer += buf;
                }
                else
                {
					ouro_snprintf(buf, 1024, "| %X ", read<uint8>(idx));
					fbuffer += buf;
                }
                ++j;
            }
