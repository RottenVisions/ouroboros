...
#define OURO_LOG4CXX_ERROR(logger, s)	\
	{	\
		try {	\
...
#define OURO_LOG4CXX_WARN(logger, s)	\
	{	\
		try {	\
...
#define OURO_LOG4CXX_FATAL(logger, s)	\
	{	\
		try {	\
...
void DebugHelper::initialize(COMPONENT_TYPE componentType)
{
	...
	{
		ouro_snprintf(helpConfig, MAX_PATH, "log4j.properties");
	...
	else
	{
		...
		std::string ouroboros_xml_path = Resmgr::getSingleton().matchRes("server/ouroboros.xml");
		if (ouroboros_xml_path != "server/ouroboros.xml")
		{
			ouro_snprintf(helpConfig, MAX_PATH, "log4cxx_properties/%s.properties", COMPONENT_NAME_EX(componentType));
			strutil::ouro_replace(ouroboros_xml_path, "ouroboros.xml", helpConfig);

			FILE * f = fopen(ouroboros_xml_path.c_str(), "r");
			if (f == NULL)
			{
				ouro_snprintf(helpConfig, MAX_PATH, "server/log4cxx_properties_defaults/%s.properties", COMPONENT_NAME_EX(componentType));
				cfg = Resmgr::getSingleton().matchRes(helpConfig);
			}
			else
			{
				fclose(f);
				cfg = ouroboros_xml_path;
			}
		}
		else
		{
...
void DebugHelper::onMessage(uint32 logType, const char * str, uint32 length)
{
	...
	{
		if(g_ouroSrvConfig.tickMaxBufferedLogs() > 0 && hasBufferedLogPackets_ > g_ouroSrvConfig.tickMaxBufferedLogs())
		{
			int8 v = Network::g_trace_packet;
			Network::g_trace_packet = 0;
...
#ifdef NO_USE_LOG4CXX
#else
	OURO_LOG4CXX_FATAL(g_logger, std::string(buf));
#endif
